...
# モデル読み込み（CI環境）
model_1 = joblib.load("keiba_model_1着.pkl")
model_2 = joblib.load("keiba_model_2着.pkl")
model_3 = joblib.load("keiba_model_3着.pkl")

# 過去データ読み込み
df_all = pd.read_csv("oi_all.csv")
...
if len(results) == 0:
    ...
    filename = f"articles/note_{date_str}.md"
    ...
    with open(f"articles/x_post_{date_str}.txt", "w", encoding="utf-8") as f:
        f.write(x_post)
    print(f"\n=== X投稿文 ===\n{x_post}")